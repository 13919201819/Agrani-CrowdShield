...
export async function generateStaticParams() {
  try {
    ...
    return drones.map((d: any) => ({ id: d.id }));
  } catch (error) { return []; }
}

export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params; // Unwrapping the Promise (Fixes error)
  const res = await fetch(`${API_BASE}/catalog/`);
  const data = await res.json();
  const drone = data.find((d: any) => d.id === id);

  return <DroneSpecsClient drone={drone} id={id} />;
}