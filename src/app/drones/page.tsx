...
export default function DroneCatalog() {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/catalog/`)
      ...
      .then((data) => {
        ...
        const formattedDrones = data.map((d: any) => ({
          id: d.id,
          name: d.name,
          category: d.tagline, // API field 'tagline' -> UI 'category'
          price: d.price,
          image: d.image_url,  // API field 'image_url' -> UI 'image'
          features: d.features,
          specs: {
            range: d.range,
            wind: d.wind,
  ...
  return (
    <main className="drone-page">
      {/* HEADER RESTORED */}
      <div className="catalog-header" style={{ padding: '60px 40px 20px', textAlign: 'center' }}>
        <p style={{ color: '#3b82f6', fontSize: '10px', letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: '10px' }}>
          Department of Defense • Authorized Catalog
        </p>
        <h1 style={{ color: '#fff', fontSize: '48px', fontWeight: '800', margin: 0 }}>
